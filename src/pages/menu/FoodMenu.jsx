...
const FOOD_SLUGS = [
  'soups', 'noodles', 'appetizers', 'gunkan', 'temaki', 'oshi',
  'nigiri', 'sashimi', 'maki', 'special-rolls', 'fried-rolls',
  'dynamite', 'fire-rolls', 'poke', 'combos',
];
    ...
    Promise.all([
      ...
      if (prodData.status === 'success') {
        const filtered = prodData.data.products.filter((p) => {
          ...
          return cat && FOOD_SLUGS.includes(cat.slug);
        });
        setProducts(filtered);
      }
      if (catData.status === 'success') {
        const foodCats = catData.data.categories.filter((c) => FOOD_SLUGS.includes(c.slug));
      ...
      <div className="max-w-lg mx-auto px-5 py-6 sm:py-10">
        ...
        <div className="flex items-center gap-3 mb-8 animate-fade-in">
          ...
          <div>
            ...
            <h1 className="font-serif text-2xl sm:text-3xl text-white tracking-tight">Food</h1>
          </div>
        </div>
        ...
        <div className="space-y-8 animate-slide-up" style={{ animationDelay: '0.15s' }}>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-16">
              <p className="text-white/20 text-sm">No food items found</p>
            </div>
          )}
        </div>
      </div>
