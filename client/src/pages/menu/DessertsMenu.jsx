...
const DESSERT_SLUGS = ['dessert', 'waffle'];
    ...
    Promise.all([
      ...
      if (prodData.status === 'success') {
        const filtered = prodData.data.products.filter((p) => {
          ...
          return cat && DESSERT_SLUGS.includes(cat.slug);
        });
        setProducts(filtered);
      }
      if (catData.status === 'success') {
        const dessertCats = catData.data.categories.filter((c) => DESSERT_SLUGS.includes(c.slug));
        setCategories(dessertCats);
      }
      setIsLoading(false);
    });
  }, []);
      ...
      <div className="max-w-lg mx-auto px-5 py-6 sm:py-10">
        ...
        <div className="flex items-center gap-3 mb-8 animate-fade-in">
          ...
          <div>
            ...
            <h1 className="font-serif text-2xl sm:text-3xl text-white tracking-tight">Desserts</h1>
          </div>
        </div>
        ...
        <div className="space-y-8 animate-slide-up" style={{ animationDelay: '0.15s' }}>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-16">
              <p className="text-white/20 text-sm">No desserts found</p>
            </div>
          )}
        </div>
      </div>
